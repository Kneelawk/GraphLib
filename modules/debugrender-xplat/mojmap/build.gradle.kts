...
val mojmapJar = tasks.create("mojmapJar", RemapJarTask::class) {
    ...
    dependsOn(project(":debugrender-xplat").tasks.remapJar)

    inputFile.set(project(":debugrender-xplat").tasks.remapJar.flatMap { it.archiveFile })
...
val mojmapSourcesJar = tasks.create("mojmapSourcesJar", RemapSourcesJarTask::class) {
    classpath.from((loom as LoomGradleExtension).getMinecraftJarsCollection(MappingsNamespace.INTERMEDIARY))
    dependsOn(project(":debugrender-xplat").tasks.remapSourcesJar)

    archiveClassifier.set("sources")

    inputFile.set(project(":debugrender-xplat").tasks.remapSourcesJar.flatMap { it.archiveFile })
    sourceNamespace.set("intermediary")
    targetNamespace.set("named")
